...
#define X_MAX 11							//ширина карты
#define Y_MAX 15							//высота карты
...
private:
	...
	void findPath() {									//ищет путь к objective, трансформирует map, записывает путь в path в ввиде последовательности клеток пути в обратном порядке
		...
		while (!queue.empty()) {
			...
			position pos;
			if (iter_pos.x == objective.x && iter_pos.y == objective.y) {
				found = 1;
		...
		if (found == 1) {
			while (map[iter_pos.x][iter_pos.y] != 1) {
				position pos;
				if ((iter_pos.x - 1) > -1 && map[iter_pos.x - 1][iter_pos.y] == map[iter_pos.x][iter_pos.y] - 1) {
					pos.x = iter_pos.x - 1;
					pos.y = iter_pos.y;
				}
				...
				if ((iter_pos.y - 1) > -1 && map[iter_pos.x][iter_pos.y - 1] == map[iter_pos.x][iter_pos.y] - 1) {
					pos.x = iter_pos.x;
					pos.y = iter_pos.y - 1;
				}
...
public:
	...
	void go() {											//погрузчик делает следующий шаг	
		...
		++cur_path_len;
		if (next_step.x == objective.x && next_step.y == objective.y) {
			objective.x = X_MAX - 1;
			objective.y = Y_MAX - 1;
			findPath();
		}
